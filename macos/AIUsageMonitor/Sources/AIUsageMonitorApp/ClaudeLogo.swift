...
enum ClaudeLogo {
    ...
    static let parsedSegments = SVGPathParser.parse(svgPath)

    /// Build a filled `NSBezierPath` of the starburst, scaled to fit `rect` and
    /// flipped so the SVG's Y-down space maps to AppKit's Y-up image space.
    static func bezierPath(in rect: NSRect) -> NSBezierPath {
        let scale = min(rect.width, rect.height) / viewBox
        let drawn = viewBox * scale
        // Center the (square) glyph inside the icon rect.
        let offsetX = rect.minX + (rect.width - drawn) / 2
        let offsetY = rect.minY + (rect.height - drawn) / 2

        func map(_ point: CGPoint) -> NSPoint {
            NSPoint(
                x: offsetX + point.x * scale,
                y: offsetY + (viewBox - point.y) * scale   // flip Y: SVG is top-down
            )
        }
        ...
        for segment in parsedSegments {
            ...
            case let .line(point):
                path.line(to: map(point))
            case let .curve(to: end, control1: c1, control2: c2):
                path.curve(to: map(end), controlPoint1: map(c1), controlPoint2: map(c2))
            case .close:
                path.close()
            }
        }
